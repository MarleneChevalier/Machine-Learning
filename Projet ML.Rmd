...
Dans le jeu test aussi, seule la variable "revenu" est manquante pour `r revtNA` valeurs, soit `r txrevtNA`% de valeurs manquantes.  
...
Créons une variable qui dépend de la disponibilité ou non (NA) de la variable revenu et du succès ou de l'échec de la campagne (variable cible). Cette nouvelle variable *revmv.cible* prend donc les valeurs :
...
**L'âge** présente une particularité : les personnes de plus de 70 ans et, à forte majorité, celles de moins de 20 ans n'ont pas indiqué leur revenu et sont en "échec" sur la campagne.  
...
Nous n'allons pas imputer les valeurs manquantes car elles concernent des populations soit avec peu ou pas de revenus ou soit des populations pour lesquelles un revenu moyen n'est pas représentatif (les retraites, allocations chomage varient selon la profession exercée auparavant).  
...
Une des premières étapes d'analyse exploratoire de données est de vérifier que le jeu de données est équilibré.  
...
Cela veut dire que notre jeu de données est équilibré : nous n'avons donc pas besoin d'appliquer des techniques de sous-échantillonnage ou SMOTE par exemple. 
...
La courbe du nombre d'observations de réussite forme une cloche entre 25 ans et 74 ans : pour les 40-55 ans, la campagne est quasi-totalement une réussite (haut de la cloche) ; alors que pour les populations les plus jeunes (avant 25 ans) et les plus agées (après 74 ans), l'échec sur la campagne est quasi-général. 

**Le résultat de campagne est fortement déterminé par l'age de la personne interrogée.**

## Résultat de la campagne selon la catégorie socio-professionnelle  
```{r,fig.height = 4, fig.width = 6, warning=FALSE}
...
Les catégories socio-professionnelles présentent toutes une majorité de succès, sauf les étudiants et les retraités, ce qui se rapproche des résultats de la campagne par age (ci-dessus).  
...
Le type de villes principalement interrogées sont les communes simples et les chefs-lieux de canton (73% des réponses). Les échecs de la campagne sont majoritaires dans tous les types de ville sauf 2 : les sous-préfectures et les chefs-lieux de canton.  
...
Il s'agit, d'une part, de construire le modèle de prédiction de la cible de campagne, en testant plusieurs méthodes (régression logistique, arbre de decision, random forests, boosting), d'évaluer les performances des modèles en calculant l'AUC et le taux d’erreur entre les prévisions et les vraies valeurs, puis en analysant la courbe de ROC.
...
Ensuite, nous utilisons la fonction GLM, pour faire une régression logistique. Voici le modèle que la fonction step a choisi comme le meilleur (le minimum AIC)
...
Avec un AIC de `r model2_aic`. Il semblerait que la variable "population" ne soit pas pertinente. 

**Prédiction**

Sur la base de ce modèle linéaire, voici la matrice de confusion quand le seuil est à 50% :
...
La matrice indique en ligne les valeurs prédites de la variable cible et en colonne les vraies valeurs de cette variable. Les erreurs de prédictions sont donc les valeurs prédites "FALSE" alors que les valeurs réelles sont "TRUE" et les valeurs prédites "TRUE" alors que les vraies valeurs sont "FALSE".  
Le taux d'erreur de prédiction que nous obtenons ici est de :`r err.pred1` %.  
...
misClasificError =round(mean(fitted.results != valid$cible)*100,3)
...
Sans surprise, comme nous l'avions vu dans l'analyse exploratoire, la CSP, l'âge et le genre sont les critères utilisés pour faire les splits. 
Les catégories 10 et 13 font le premier split, ensuite l'âge 32 et 61, pour finir avec le genre. Aucune autre variable n'est utilisée pour faire des splits. 
...
Pour la fonction **printcp**, chaque ligne représente une hauteur différente de l'arbre. En général, plus il y  de niveaux dans l'arbre, moins il y a d'erreurs de classification sur l'entrainement. Cependant, il existe le risque d'overfitting. Souvent, l'erreur de validation croisée augmentera à mesure que l'arbre atteindra plus de niveaux (au moins, après le niveau «optimal»). 
...
Dans notre cas ce valeur est 0.01. 
...
La courbe ROC pour l'arbre de décision atteint très rapidement la valeur de 0.8, plus rapidement que la courbe ROC pour la régression logistique. 
...
Nous allons modifier le paramètre "minsplit", il contrôle le nombre minimum d'observations qui doit exister dans un nœud pour qu'une tentative de division soit faite; nous allons aussi modifier la valeur CP pour augmenter la croissance de l'arbre de décision.

Nous allons démarrer avec le CP = 0.0001, et minsplit =2, afin d'observer la croissance de l'arbre et les différentes valeurs de CP. 

```{r}
New.tree2 = rpart(cible~.,data=train_arbre,method="class",control = rpart.control(minsplit = 2, cp = 0.0001))
```

Avec ce nouvel arbre, nous obtenons des meilleurs résultats (selon la matrice de confusion). 
...
#erreur de prédiction
tree.err2=round((tree.mc2[1,2]+tree.mc2[2,1])/sum(tree.mc2[,])*100,3)
```

Le taux d'erreur de prédiction est ici de : `r tree.err2` %.  

Cependant cet arbre peut être trop proche des valeurs d'entrainement et mal fonctionner avec les données de tests. Pour éviter le surajustement, nous devons maintenant élaguer notre arbre. 

**Elaguer l'arbre**

Pour cette étape nous allons utiliser la fonction printcp. La valeur de CP optimale est celle pour laquelle la valeur de xerror est la plus basse (cross-validated error).
...
```{r}
mincp = round(New.tree2$cptable[which.min(New.tree2$cptable[,"xerror"]),"CP"],3)
```

**La valeur optimale de CP est 0.00039**, avec un **xerror de 0.12343**.  

Dans le graphique suivant on observe bien la diminution de xerror qui atteint son minimum et ensuite remonte à nouveau (overfitting).
...
Voici notre nouvelle matrice de confusion, avec le CP optimal :
...
#erreur de prédiction
tree.err3=round((tree.mc3[1,2]+tree.mc3[2,1])/sum(tree.mc3[,])*100,3)

```

Le résultat après élagage est bien meilleur puisque le taux d'erreur de prédiction est de  : `r tree.err3`%.

**Importance des variables**

Nous allons obtenir l'importance des variables pour faire les splits de l'arbre de décision. L'importance des variables explicatives selon le critère de Gini mesure l'influence des variables pour séparer les individus selon les valeurs de la cible (echec/succès) : plus le critère de Gini sera élevé, plus la variable aura de l'influence pour distinguer le succès de l'échec de la campagne. 
...
Ce résultat graphique coincide avec ce que nous avons observé lors de l'analyse exploratoire : l'âge et la catégorie sont les variables les plus pertinentes pour effectuer la prediction de la cible.  
...
Les parametres d'une foret aléatoire sont le nombre d'arbres (ntree) et le nombre de variables explicatives testées à chaque séparation (mtry). Pour les optimiser, il faut chercher le modèle de forest aléatoire qui minimise l'erreur Out Of Bag (OOB) : il s'agit de l'erreur d'estimation moyenne sur les échantillons d'apprentissage.
...
Nous cherchons maintenant à optimiser la taille de la foret (nombre d'arbres), c'est à dire trouver la taille qui permettra de réduire l'erreur OOB.  
...
Graphiquement, on constate que le taux d'erreur est toujours légèrement oscillant avec 500 arbres et 2500 arbres. Il semble être plus stable à 5000 arbres.  

Nous comparons l' erreur OOB selon le nombre d'arbres :  
...
A 2500 arbres, l'erreur OOB est la plus faible parmi les cas testés.  

Nous fixons donc le nombre d'arbres à 2500. Essayons d'approcher maintenant le nombre optimum de variables explicatives testées à chaque séparation (mtry) . Nous testons mtry = 1 , 4 et 6 . 

```{r}
#random forest ntree=2500 mtry=1
rforest5=randomForest(cible~.,data=train_arbre,ntree = 2500,mtry = 1, na.action=na.omit)
#random forest ntree=2500 mtry=4
rforest6=randomForest(cible~.,data=train_arbre,ntree = 2500,mtry = 4, na.action=na.omit)
#random forest ntree=2500 mtry=6
rforest7=randomForest(cible~.,data=train_arbre,ntree = 2500,mtry = 6, na.action=na.omit)

rf5.oob=round(rforest5$err.rate[2500,1]*100,2)
rf6.oob=round(rforest6$err.rate[2500,1]*100,2)
rf7.oob=round(rforest7$err.rate[2500,1]*100,2)
```
...
Il apparait que le taux d'erreur est le plus bas pour mtry=4.  

Nous gardons donc les paramètres pour notre modélisation par random forest : **ntree=2500 et mtry=4.**  
...
Ici on observe que, selon le critère de Gini, l'age est la variable la plus discriminante.

**Matrice de confusion et erreur de prévision**
...
#erreur de prédiction
rf6.err=round((rf6.mc[1,2]+rf6.mc[2,1])/sum(rf6.mc[,])*100,3)
```
   
L'erreur de prédiction obtenue avec cette forêt (ntree=2500 et mtry=4) est de `r rf6.err`%.   
   
**Courbe de ROC et AUC** 
             
```{r fig.height = 3.5, fig.width = 5}

rforest6.pred = prediction(as.numeric(rforest6.pred), as.numeric(valid_arbre$cible))

rf6.roc =ROCR::performance(rforest6.pred,"tpr","fpr")
plot(rf6.roc, col="purple", lwd=2) 
lines(x=c(0, 1), y=c(0, 1), col="black", lwd=2)
```


```{r}
rf6.auc=ROCR::performance(rforest6.pred,"auc")
rf6.auc=round(rf6.auc@y.values[[1]],4)
```

**Avec un modèle de forêt aléatoire de ntree=2500 et mtry=4, on obtient un AUC est de `r rf6.auc`**


## Méthode ensembliste : le boosting - XGBOOST

Une autre méthode que nous pouvons utiliser est le XGBOOST (Extreme Gradient Boosting) ; c'est une méthode similaire au gradient boosting mais plus efficace. Elle combine des algorithmes d'apprentissage des arbres et des modèles linéaires. En plus, elle permet de faire des calculs parallèles sur une seule machine, ce qui la rend très rapide. 
...
Avant d'utiliser l'algorithme xgboost nous devons preparer les données, XGBoost fonctionne uniquement avec des vecteurs numériques. C'est-à-dire, nous devons transformer toutes nos variables utiliser en nombre, dont sex, categorie, reg et city.type. 
...
1- Transformation des Variables vecteurs numériques
...
Nous allons utiliser le one hot encoding pour créer des variables "dummy" et ainsi convertir les variables en numériques. Par exemple, pour le genre il fera deux variables : SexFemale et SexMale, et donnera la valeur de 1 sur la colonne SexFemale quand l'individu est de sexe féminin et zéro dans la colonne SexMale.
...
2 - Séparer la variable à prédire  

La deuxième étape consiste à séparer la variable cible du reste de la matrice.
...
La troisième étape consiste à convertir nos données en format Dmatrice. Cette étape n'est pas nécessaire, mais elle va faire que l'entrainement soit plus rapide, et c'est obligatoire si on souhaite utiliser plusieurs coeurs. Nous utilisons pour le faire la fonction xgb.DMatrix. 
...
Afin d'entrainer notre modèle, Xgboost a besoin de connaître : 

1- Les données à utiliser pour l'entrainement, notre dmatrix dtrain   
...
```{r}
model.boost = xgboost(data = dtrain, # the data   
...
Nous pouvons maintenant lancer notre entrainement avec la fonction classique *predict*. 

```{r}
# prediction
# prediction
pred.boost = predict(model.boost, dtest)

xgb.err1 = round(mean(as.numeric(pred.boost > 0.5) != ts_label)*100,3)
print(paste("erreur de test =", xgb.err1,"%"))

```

L'erreur de test reste très proche à l'erreur d'entrainement, cela nous indique qu'il n'y a pas eu de surajustement. 
...
#matrice de confusion
xgb.mc=table(pred_label, ts_label)

VrVal=factor(c("Echec", "Echec", "Succès", "Succès"))
PrVal=factor(c("Echec", "Succès", "Echec", "Succès"))
Y=xgb.mc
df.xgb.mc=data.frame(VrVal, PrVal, Y)
ggplot(data = df.xgb.mc, mapping = aes(x = VrVal, y = PrVal)) +
  geom_text(aes(label = sprintf("%1.0f", Y)), vjust = 1) +
  labs(x = "Vraies Valeurs", y = "Valeurs prédites") +
  theme_bw() 
...
Nous pouvons modifier quelques paramètres pour améliorer notre modèle :

* **max.depth** = 6 la profondeur maximale de chaque arbre de décision
* **nrounds** = 100 nombre maximum d'itérations de boost
...
model_tuned = xgboost(data = dtrain, # the data           
                 ...
                 verbose=0) # the objective function 

# generate predictions for our held-out testing data
pred.boost2 = predict(model_tuned, dtest)

# erreur de classification
xgb.err2 = round(mean(as.numeric(pred.boost2 > 0.5) != ts_label)*100,3)
print(paste("erreur de test =", xgb.err2,"%"))
```

Uniquement avec deux paramètres, notre erreur a baissé. Cependant nous avons d'autres paramètres qui peuvent être ajustés, comme :

* **eta** = Taille de chaque étape de boosting
* **nfold** = l'ensemble de données d'origine est partitionné de manière aléatoire en nfold sous-échantillons de taille égale
* **nthread**= nombre de threads utilisés dans la formation, s'ils ne sont pas définis, tous les threads sont utilisés

Par rapport aux résultats obtenus, nous n'allons pas modifier ces paramètres
...
# graphique
xgb.plot.importance(importance_matrix, col="cyan")
```

Comme les autres modèles, l'âge est la variable la plus pertinente ; il apparait aussi que la modalité "sexe - féminin " est influente pour la construction de l'arbre XGboost.  
...
plot(xgb.roc, col="blue", lwd=2,main="Courbe ROC") 
lines(x=c(0, 1), y=c(0, 1), col="black", lwd=2)
```
...
**Avec un modèle de xgboost, on obtient un AUC est de `r xgb.auc`**


## En conclusion

Les résultats des différentes méthodes testées sont les suivants :  

 Méthodes testées       | AUC         | Erreur de prédiction |
----------------------- |-------------|----------------------|
 Regression logistique  |`r auc.pred1`|`r misClasificError` %|     
 Arbre de décision      |`r tree2.auc`|`r tree.err3` %       |             
 Forêt aléatoire        |`r rf6.auc`  |`r rf6.err` %         |             
 Boosting               |`r xgb.auc`  |`r xgb.err2` %        |    
...
plot(prf3,col="hotpink", lwd=2, main="Courbe ROC")
par(new = T)
plot(rf6.roc, col="purple", lwd=2, main="Courbe ROC") 
par(new = T)
plot(xgb.roc, col="blue", lwd=2,main="Courbe ROC") 
legend("bottomright", legend=c("Régression linéaire", "Arbre décision", "Forêt aléatoire","Boosting"),
       col=c("orange", "hotpink", "purple","blue" ), lty=1, cex=0.7)

```

**La modélisation de la variable cible par la méthode Xgboost apporte ici le meilleur résultat.**   
En effet, l'AUC du boosting est le plus proche de 1 (`r xgb.auc`), ce qui s'illustre par une courbe de ROC qui tend le plus rapidement vers 1 (graphiquement: la plus proche du coin supérieur gauche). 

Nous choisissons donc de faire la prédiction à partir du modèle de boosting.

```{r}
#chargement des jeux de données d'entrainement et test
test2 = subset(test, select=-c(insee.code,f_name,last.name,commune, latitude, department, longitude,X,Y,REGION))

setDT(test2)

# selectionner les variables numeriques
test_numeric =test2 %>% select_if(is.numeric)

#utiliser le one hot encoding pour les variables de train
sex3=model.matrix(~sex-1,test2)
city3=model.matrix(~city.type-1,test2)
reg3=model.matrix(~reg-1,test2)
cat3=model.matrix(~categorie-1,test2)

#Les fusionner pour faire une seule matrice
test_numeric = cbind(test_numeric,sex3,city3,cat3,reg3)
test_matrix = data.matrix(test_numeric)

#Prediction et creation du fichier csv
pred.boost3 = predict(model_tuned, test_matrix)
pred.boost_final = ifelse(pred.boost3 > 0.5, 1, 0)
write.csv(pred.boost_final,"test_results.csv")
...
Selon le graphique, il y a que 4 catégories sans revenu (10, 11, 12 et 13) : étudiants, chômeurs, inactifs et retraités. Sur le reste des catégories, nous ne trouverons pas des données absentes.  
...
## Annexe - Regression linéaire - Graphique des résidus 
...
## Annexe - XGBOOST - Graphique de l'arbre

```{r, warning=FALSE}
xgb.plot.multi.trees(feature_names = names(train_numeric), model = model_tuned )
